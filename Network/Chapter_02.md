...
- 서브넷에 연결되어 있는 기기들 전체를 대상으로 해당 IP와 일치하는 기기를 찾는다. (브로드캐스트)
- 해당 IP에 해당하는 기기는 자신의 MAC 주소를 알려준다.
- ARP 동작이 많아지는 것을 방지하기 위해 한번 조사한 MAC 주소를 ARP 캐시에 저장하여 1차적으로 캐시에 저장되어 있는지 검사하고 ARP를 수행한다
- 캐시는 IP 주소 설정으로 고쳐지는 경우 유효하지 않을 수 있기 때문에 수 분내로 삭제됨
- 이후 MAC 헤더를 IP 헤더 앞에 붙이면 패킷이 완성됨
- 여기까지가 IP 담당 부분의 역할임, 이후엔 LAN 어댑터가 전송하면 됨
#### 6. 이더넷 기본
- 이더넷은 다수의 컴퓨터가 여러 상대와 자유롭게 적은 비용으로 통신하기 위해 고안된 기술
- 네트워크는 케이블로 이루어져 있고 컴퓨터가 데이터를 송신하면 데이터가 케이블을 통해 네트워크 전체에 흐르게 되어 전체에게 신호가 도착
- 신호가 누구에게 갈 것인지 판단하기 위해서 헤더 부분에 수신처의 주소를 작성
- 주소에 해당하는 기기는 패킷을 수신하고 다른 기기는 패킷을 폐기함
- 이 동작을 제어하기 위해서 MAC 헤더를 사용함, 주소
- 현재는 스위칭 허브를 사용한 형태가 보급되었는데 허브에서 MAC 헤더의 MAC 주소를 보고 목적지 기기로만 신호를 흘려보냄
- 이더넷은 MAC 헤더의 수신처 MAC 주소로 기억된 상대에게 패킷을 전달하고 송신처 MAC 주소로 송신처를 나타낸 후 이더 타입으로 패킷의 내용물을 나타낸다
#### 7. IP 패킷을 전기나 빛의 신호로 변환하여 송신
- 디지털 데이터를 전기나 빛의 신호로 변환해서 네트워크 케이블에 송출
- LAN 어댑터는 LAN 드라이버에 의해 제어되며 기본적으로 ROM에 기록된 MAC 주소를 사용하지만 드라이버에서 직접 설정 가능함
#### 8. 패킷에 3개의 제어용 데이터 추가
- LAN 드라이버는 패킷을 받으면 LAN 어댑터의 버퍼 메모리에 복사, 송신 명령을 보내면 LAN 어댑터 내의 MAC 회로가 동작
- MAC 회로는 맨 앞에 프리앰블과 스타트 프레임 딜리미터(SFD)를 맨 뒤에는 프레임 체크 시퀀스(FCS)라는 오류 검출용 데이터를 부가함
- 디지털 데이터를 전기 신호로 바꾸면 0과 1 비트값을 전압이나 전류에 대응시키는데 신호가 0이나 1로 연속되면 비트 구분이 어려워서 데이터를 나타내는 신호와 비트 구분을 위한 클록 신호를 합성해서 한개의 신호로 보내게 됨
- 클록 신호의 타이밍을 알 수 있으면 데이터 신호를 추출할 수 있으므로 클록신호가 변화하는 타이밍을 잡기위한 신호를 패킷에 부가하는데 이게 프리앰블임
- 스타트 프레임 딜리미터는 0과 1을 반복해서 프리앰블이 규칙적으로 신호를 보내다가 마지막에 변칙적으로 11 같은 식으로 파형을 변화시켜서 패킷의 개시 위치를 알려주는 역할을 함
- FCS는 패킷을 운반하는 도중 잡음의 영향으로 데이터가 변한 경우 검출하기 위함
- 32비트의 비트열이고 패킷 데이터를 CRC라는 오류 검사 계산식을 적용한 값으로 데이터가 변하면 계산한 결과도 바뀌게 되어 수신측에서 계산한 FCS가 송신측에서 보낸 FCS와 같은지 검사하는 것으로 오류 검출이 가능함
#### 9. 허브를 향해 패킷을 송신
- 프리앰블, 스타트 프레임 딜리미터, FCS를 부가하면 케이블에 송출하는 패킷이 완성됨.
- 송출 동작에는 리피터 허브를 사용한 반이중 모드, 스위칭 허브를 사용한 전이중 모드가 있음
- 먼저 반이중 모드는 송신과 수신을 한 시점에 한 쪽 기능만 수행할 수 있는 것으로 신호의 충돌을 막기위해 케이블에 다른 신호가 있는지 확인하고 흐르지 않을 때 송신을 시작함
- 이 동작은 MAC 회로가 프리앰블의 맨 앞부터 1비트씩 차례대로 전기신호로 변환하고 MAU 또는 PHY라는 송, 수신 신호 부분에 보냄
- 이 떄 디지털 데이터를 전기 신호로 변환하는 속도가 전송 속도임, 1초 동안 10메가비트를 변환하면 10메가비트/초 전송률
- PHY(MAU) 회로는 이 신호를 케이블에 송출하는 형식으로 변환함. PHY(MAU) 회로는 MAC 회로가 송신한 신호의 형식을 변환하기 위한 회로
- 이더넷에는 다양한 파생 형식이 있고 케이블에 송출하는 신호의 형식에는 많은 변형이 있음
- 이더넷은 송신한 신호가 도착했는지 확인하지 않음 (잘 일어나지도 않고 TCP가 검출하기 때문)
- PHY(MAU) 회로는 케이블에 신호가 흐르지 않을 때 송신을 시작함
- 만약 송신중 수신 신호가 흘러들어오면 반이중 모드의 경우 신호가 섞여서 충돌 상태가 됨
- 충돌 상태에선 송신을 중지하고 충돌 사실을 재밍 신호로 다른 기기에 알림
- 잠시 대기했다가 재송신을 하는데 충돌을 일으킨 기기와 대기시간이 동일하면 다시 충돌할 가능성이 있으므로 대기시간은 난수로 결정됨
- 계속 충돌하면 대기시간을 늘리다가 충돌이 해결되지 않으면 오류로 판단함
- 전이중 모드는 송, 수신을 동시에 실행하면서 충돌이 발생하지 않음
#### 10. 돌아온 패킷을 받음
- 리피터 허브를 이용한 반이중 동작의 이더넷에는 1대가 송신한 신호가 리피터 허브에 접속된 케이블 전부에 흘러감
- 신호의 맨 앞에는 프리앰블이 있으므로 파형에서 타이밍을 계산하여 스타트 프레임 딜리미터가 나오면, 그 다음 비트 부터 디지털 데이터로 변환해서 동작을 수행함, 이 과정은 송신과 반대
- PHY(MAU) 회로에서 신호를 공통 형식으로 변환해서 MAC 회로에 보내고 MAC 회로에서 신호를 디지털 데이터로 변환해서 버퍼 메모리에 저장
- 신호의 마지막에 다다르면 받은 데이터의 FCS를 계산하고 수신한 FCS와 일치하는지 검사해서 오류 패킷이면 폐기함
- FCS에 문제가 없으면 MAC 헤더의 수신처 MAC 주소를 조사해서 자신의 LAN 어댑터를 초기화할 떄 설정한 자체 MAC 주소와 비교해서 자신에게 온 패킷인지 판단함
- 다른 목적지의 패킷이면 폐기하고 자신에게 오는 것이면 패킷을 버퍼 메모리에 저장하고 패킷을 수신한 사실을 컴퓨터 본체에게 통지
- 이 통지는 인터럽트라는 구조를 사용하는데 송, 수신 동작을 하고 있으면 컴퓨터 본체는 LAN 어댑터를 감시하고 있는게 아니라 다른 작업을 수행하고 있음
- 그래서 LAN 어댑터가 확장 버스 슬롯 부분의 인터럽트용 신호선에 신호를 보내면 본체측의 인터럽트 컨트롤러를 통해 CPU에게 전달이 되고 CPU는 실행하던 다른 작업을 보류하고 OS 내부의 인터럽트 처리용 프로그램 쪽으로 전환함
- 여기에서 LAN 드라이버가 호출되어 LAN 어댑터를 제어하면서 송, 수신 동작을 실행
- LAN 드라이버가 동작해서 LAN 어댑터의 버퍼 메모리에서 수신한 패킷을 추출하면 MAC 헤더의 타입필드를 보고 프로토콜을 판별함.
- 예를들어 타입이 0800이면 IP 프로토콜의 데이터이므로 TCP/IP의 프로토콜 스택에 패킷을 건네줌
#### 11. 서버의 응답 패킷을 IP에서 TCP로 넘김
- TCP/IP 프로토콜 스택에서 IP 담당 부분은 IP 헤더 부터 조사해서 IP 주소를 추출하고 자신의 IP와 일치하는지 확인
- IP가 다르거나 오류가 발생하면 ICMP를 사용하여 통신 상대방에게 오류를 통지함
- 수신처 IP가 올바르면 수신하는데 한 가지 일이 더 있음
- IP 프로토콜에는 프래그멘테이션이라는 기능이 있는데 통신 회선이나 LAN 중에는 짧은 패킷만 다룰 수 있는 것이 있음
- 따라서 패킷을 짧게 하기 위해 패킷을 여러개로 분할하는 경우가 있는데 만약 수신한 패킷이 분할한 것이면 IP 담당 부분이 이를 되돌려야함
- 분할된 패킷은 IP 헤더의 플래그 항목을 보면 알 수 있으므로 패킷이 분할된 것이면 IP 담당 부분의 메모리에 일시적으로 보관하고 IP 헤더의 ID 정보가 같은 패킷들이 도착하기를 기다림
- 또한 프래그먼트 오프셋이라는 항목에는 분할되기 전에 패킷의 위치가 나타나므로 이 정보를 바탕으로 분할된 패킷들을 원래 패킷으로 조립함 (리어셈블링)
- 이 과정까지 끝마치면 IP 담당 부분의 역할은 끝나므로 TCP 담당부분에 건네줌
- 그러면 TCP 담당 부분은 IP 헤더의 송, 수신처 IP주소와 TCP 헤더의 송, 수신처 포트 번호를 조사해서 해당하는 소켓을 찾음
- 소켓에는 통신 진행 상황(연결 수립/해제, 연결된 상태)이 기록되어 있으므로 상황에 맞는 적절한 동작 수행
### 6. UDP 프로토콜 송, 수신 동작
#### 1. 수정 송신이 필요없는 데이터의 송신은 UDP가 효율적
- TCP가 복잡하게 설계된 이유는 데이터가 도착하지 않으면 다시 보내야 하는데 데이터 전부를 다시보내는건 비효율적이므로 도착하지 않은 패킷에 대해서만 다시 보내기 때문에 복잡함
- 만약 데이터가 패킷 하나에 들어갈 만큼 짧은 데이터면 '전부' 다시 보낸다해도 이 패킷하나만 보내면 되므로 전혀 비효율적이지가 않음
#### 2. 제어용 짧은 데이터
- DNS 서버에 대한 조회 등 제어용으로 실행하는 정보 교환은 한 패킷으로 끝나는 경우가 많으므로 UDP를 사용
- 수신 확인이나 윈도우가 없어서 접속, 연결 끊기 단계가 없음, 애플리케이션으로부터 데이터를 받으면 UDP 헤더를 부가하고 IP에 의뢰하여 송신하기만 함
- 수신은 IP 헤더의 송, 수신처 IP 주소와 UDP 헤더의 송, 수신처 포트 번호를 보고 건네줄 애플리케이션을 판단하여 건네주기만함
- 패킷이 없어져도 별다른 처리를 안하고 송신측에서도 회답이 돌아오지 않으면 한 번 더 다시 보낼뿐임
#### 3. 음성 및 동영상 데이터
- 음성, 영상 데이터도 UDP를 사용함. (데이터 도착이 지연되면 재생하는 타이밍이 맞지않아 음성이 끊기거나 영상이 멈추기 때문)
- 재생 타이밍이 맞지 않으면 데이터를 다시 보낸다해도 쓸모가 없음 (끊긴 음성이나 영상을 어떻게 할 수가 없음)
- 음성, 영상은 데이터가 다소 없어져도 치명적이지 않음 (살짝 끊기거나 멈출 뿐이라 어느정도 허용가능) 
