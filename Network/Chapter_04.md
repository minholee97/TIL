...
#### 4. 액세스 회선 전체의 동작
- 사용자 측에 인터넷 접속용 라우터에 프로바이더가 할당한 사용자명과 패스워드를 등록하면 인터넷 접속용 라우터는 PPPoE의 브로드캐스트 방식으로 BAS를 찾음 (이 방식을 Discovery라 함)
- BAS의 MAC 주소를 얻어와서 사용자명과 패스워드를 보내는데 암호화를 하는 방식(CHAP)과 하지않는(PAP)방식 중 선택해서 전송함
- BAS에서 인증서버로는 RADIUS라는 프로토콜을 사용하여 암호화함
- 인증이 성공하면 BAS에서 사용자에게 TCP/IP의 설정 정보를 통지하는데 통지 내용은 기기에 할당한 IP 주소(글로벌), DNS 서버의 IP 주소, 기본 게이트웨이의 IP 주소를 인터넷 접속용 라우터가 받아서 설정함
- 이렇게 인터넷 접속용 라우터의 BAS측 포트에 글로벌 주소가 할당되고 라우팅 테이블에 기본 게이트웨이가 설정됨으로써 인터넷에 패킷을 중계할 수 있는 상태가 됨
- 이제 클라이언트에서 브라우저로 인터넷에 엑세스하면 패킷이 흘러올 것이고 이 패킷의 수신처는 인터넷의 어느 한 부분이므로 아마 라우팅 테이블에 등록되어있지 않을 것임
- 따라서 등록했던 기본 게이트웨이로 패킷을 송신함
- 패킷을 송신하는 부분은 PPPoE의 규칙을 따름, 패킷에 MAC 헤더를 붙이는데 수신처 MAC 주소는 PPPoE의 Discovery에서 조사한 BAS의 MAC 주소를, 송신처 MAC 주소는 인터넷 접속용 라우터의 BAS측 포트의 MAC 주소를, 그리고 이더 타입은 PPPoE를 나타내는 8864(16진수)
- 다음에 PPPoE 헤더와 PPP 헤더는 버전이나, 타입, 코드 등 사전에 대부분 결정되어있는 값들로 구성되어 있음
- 3개의 헤더를 붙인 뒤 BAS를 향해 패킷을 신호로 변환하여 포트에서 송신함
- BAS에 도착한 패킷에서 MAC 헤더와 PPPoE 헤더를 제거하고 PPP 헤더 이후의 부분을 추출한 후 터널링의 원리로 패킷을 송신하면 PPP 패킷은 프로바이더의 라우터에 도착함
#### 5. IP 어드레스를 할당하지 않는 언넘버드
- 이것이 PPPoE의 동작인데 BAS와 라우터는 하나의 케이블로 1대1로 이어져 있기 때문에 접속된 포트에 IP주소를 할당하지 않아도 됨 => 언넘버드
#### 6. 인터넷 접속용 라우터에서 프라이빗 주소를 글로벌 주소로 변환
- BAS는 사용자측에게 TCP/IP 정보를 통지하고 이 정보를 PC에 설정하면 글로벌 주소가 할당되어 주소 변환없이 인터넷에 액세스할 수 있는데 이것이 TCP/IP의 본래 방법임
- 하지만 인터넷 접속용 라우터를 사용하면 BAS가 통지한 정보를 라우터가 받아버려서 글로벌 주소가 라우터에 할당되어 버림
- 이 경우 PC는 프라이빗 주소를 할당하고 PC가 보낸 패킷은 인터넷 접속용 라우터에서 주소 변환 후 인터넷에 중계됨
- 웹이나 메일은 크게 상관없는데 일부 애플리케이션은 자신의 IP 주소를 통신 상대에게 통지하는 것이 있는데 주소 변환 구조가 통지 동작을 지원할 수 없는 경우가 있음
- 주소 변환의 영향으로 제대로 동작하지 않는다면 라우터를 사용하지 않고 BAS가 통지하는 정보를 PC가 받는 형태로 선택하면 됨
- 그러면 PC에 글로벌 주소를 할당할 수 있어서 인터넷에 액세스가 가능함
- 라우터를 사용하지 않는 방법에는 주의할 점이 있음, 글로벌 주소를 할당한 PC에 인터넷에서 패킷이 직접 도착하므로 공격받을 가능성이 있음(클라이언트 방화벽 추천)
#### 7. PPPoE 이외의 방식
- PPPoA는 PPP의 메시지를 이더넷의 패킷에 그대로 저장함(셀로 분할하지 않음)
- MAC 헤더와 PPPoE 헤더 역시 붙이지 않기 때문에 PPP 메시지를 그대로 이더넷에 전송할 수 없음
- BAS와 PC(또는 라우터)를 일체화 시키는 방법을 사용
- ADSL 모뎀을 라우터와 일체화 시켜서 PPPoE에서 라우터를 사용하는 방식처럼 사용함
- PPPoA는 헤더가 줄어서 MTU가 PPPoE보다 큼
- PPPoE는 효율성이 떨어질 가능성이 있고, PPPoA는 ADSL 모뎀과 라우터를 분리할 수 없다는 제약이 있음
- 문제는 PPP가 원인이므로 이를 안쓰는 액세스 회선의 사업자도 있음
- PPP 대신 DHCP라는 구조를 사용해서 TCP/IP의 정보를 BAS에서 사용자에게 통지하는 것
- DHCP는 주로 사내 LAN에서 클라이언트 PC에 TCP/IP 정보를 통지하기 위해 사용함
- 단순히 패킷을 주고받기 때문에 PPP의 추가 헤더가 붙어있지 않음, ADSL 모뎀과 라우터의 분리가 가능하고 셀도 사용하지 않고 이더넷 패킷을 ADSL 신호로 변환함.
- DHCP는 결론적으로 PPP를 사용하지 않고 이더넷의 패킷을 그대로 ADSL 신호로 변환하여 DSLAM에 송신하는 방식임
### 4. 프로바이더의 내부
#### 1. POP과 NOC
- 액세스 회선을 통과한 패킷은 프로바이더의 라우터에 도착하고 인터넷의 내부로 들어감
- 인터넷은 다수의 프로바이더의 네트워크를 서로 접속한 것이고 ADSL이나 FTTH의 액세스 회선을 통과한 패킷은 프로바이더의 POP에 설치된 라우터에 도착
- POP의 구성은 액세스 회선의 종류에 따라 여러 유형의 라우터들로 구성됨
- 전용 회선을 이용한 액세스 회선은 본인 확인이나 설정 값 통지가 필요없음 (고정적으로 접속하기 때문에)
- 전화 회선이나 ISDN 회선을 이용하는 다이얼업 회선은 RAS라는 라우터를 사용함, RAS에 사용자로부터의 전화를 받는 기능이 있기 때문에
- PPPoE나 PPPoA는 BAS가 창구역할을 해주기 때문에 보통의 라우터를 사용함
- 엑세스 회선을 연결하는 부분은 접속하는 액세스 회선이 많으므로 라우터에 다수의 포트가 필요함
- 허나 액세스 회선의 속도가 백본보다 느리기 때문에 흐르는 패킷의 양이 적음
- 엑세스 회선을 연결하는 부분 반대편에는 프로바이더의 중심인 NOC이나 다른 POP에 접속하기 위한 라우터가 있고 이 부분은 다수의 라우터에서 나오는 패킷이 모임
- NOC은 프로바이더의 핵심으로 패킷이 모이는 장소
- 이곳에서 목적지 근처에 있는 POP이나 다른 프로바이더로 패킷이 흘러가기 때문에 초 고성능의 라우터를 사용함
- NOC에도 액세스 회선 접속용 라우터를 설치하고 POP과 겸용하는 경우가 많으므로 NOC과 POP을 엄밀하게 구분할 수는 없음
#### 2. 건물 밖은 통신 회선 등으로 접속
- POP나 NOC는 전국 각지에 있고 서버룸이랑 비슷함
- 프로바이더는 통신 회선(광섬유 포함)을 사용해서 멀리 떨어진 NOC이나 POP을 연결함
### 5. 프로바이더를 경유하여 흐르는 패킷
#### 1. 프로바이더끼리 접속
- 최종 목적지에 해당하는 웹 서버가 클라이언트와 같은 프로바이더에 접속되어 있는 경우, 프로바이더의 라우터에는 라우터끼리 경로 정보를 교환하고 이것을 라우팅 테이블에 자동 등록함
- 라우팅 테이블에 검색하는 것으로 중계 대상을 찾을 수 있고 대상이 NOC이거나 POP일 수도 있는데 어느 경우든지 패킷을 대상에게 보내면 거기에도 라우터가 있어서 같은 방법으로 패킷을 중계
- 동작을 반복하다보면 패킷은 웹 서버측의 POP에 설치한 라우터에 도착하고 거기서 웹 서버를 향함
- 웹 서버측의 프로바이더가 클라이언트와 다른 경우에는 패킷을 서버측의 프로바이더에 보내야 하는데 이때 대상도 라우팅 테이블에 등록되어 있음
- 프로바이더는 다른 프로바이더와도 경로 정보를 교환하므로 라우터에는 다른 프로바이더의 경로 정보가 등록되어 있음, 따라서 경로대로 중계 대상이 발견되기 때문에 패킷을 보내기만 함
- 결국 인터넷의 라우터에는 최종 목적지가 어떤 프로바이더인지 상관 없이 모든 경로가 등록되어 있으므로 라우팅 테이블에서 다음 중계 대상을 찾아 패킷을 보내는 동작을 반복하면 그 중에서 웹 서버의 POP에 도착하게 되어있음
#### 2. 프로바이더끼리 경로 정보 교환하기
- 프로바이더 끼리 서로에게 경로 정보를 가르쳐주고 라우팅 테이블에 등록하는 작업을 수행하는데 이것을 BGP라고 함
- 이 방식은 트랜지트, 비트랜지트(피어) 두 가지 방식이 있음
- 트랜지트는 어떤 프로바이더 A가 프로바이더 B에게 A가 알 수 있는 모든 인터넷의 경로를 전부 통지하는 것으로 B는 앞으로 A를 통해 모든 프로바이더에게 패킷을 보낼 수 있게 됨
- 비트랜지트는 두 프로바이더가 각각의 네트워크에 관한 경로 정보만 상대에게 통지하는 것으로 서로 상대의 네트워크로 갈 패킷만 흐름
#### 3. 사내 네트워크에서 자동 등록
- 사내 네트워크에서 라우터끼리 서로 경로 정보 공유를 통해 라우팅 테이블을 자동 설정하는 방법을 사용함
- 사내에서 사용하는 구조는 목적지까지 최단 경로를 찾아 거기에 패킷을 중계하도록 되어있음, 따라서 전체 라우터와 무차별적으로 경로 정보를 교환함
- 프로바이더에서 이러한 방식을 사용하면 비용 부담이 커짐 (어딘가에서 온 패킷이 회선에 유입될 수 있음), 의도하지 않은 상대로부터 오는 패킷을 중지함
- 사내 네트워크는 무차별적으로 모든 라우터와 경로 정보 교환을 하지만 프로바이더는 특정 라우터와 1대 1로 경로 정보 교환을 함
- 이렇게 해서 비용 부담 등의 교섭에 응한 상대만 경로 정보를 통지할 수 있고 경로 정보를 통지한 상대 외의 패킷은 흘러들어오지 않음
- 프로바이더는 다수의 프로바이더와 연결되어 있으므로 어떻게든 경로를 선택해서 보낼 수 있도록 되어있음
#### 4. IX의 필요성
- 모든 프로바이더가 서로 1대 1로만 연결하면 통신 회선이 너무 많아짐
- IX는 이 곳을 경유해서 다른 프로바이더와 연결할 수 있도록 함
#### 5. IX에서 프로바이더끼리 접속하는 모습
- IX 중심에는 LAN 인터페이스를 다수 장착한 레이어 2 스위치가 있음
- 레이어 2 스위치의 역할은 스위치랑 동일함, 거대한 스위칭 허브가 IX의 핵심임
- 프로바이더를 라우터에 연결하는 방법은 다음 4가지가 있음
- 1) IX와 같은 건물에 프로바이더가 있다면 프로바이더의 라우터를 광섬유 케이블을 IX의 스위치에 직접 연결하는 방법
- 2) 프로바이더의 라우터를 통신 회선이나 다크 파이버 등으로 스위치에 접속하는 방법
- 3) 프로바이더가 라우터를 IX에 가지고 들어가 스위치에 접속하는 방법
- 4) 데이터센터처럼 패킷이 집중되는 장소의 스위치와 IX의 스위치를 통신 회선 등으로 연결하는 방법
- IX의 스위치는 스위치와 같이 동작하므로 라우터에서 패킷을 송신할 때 ARP에서 중계 대상의 라우터의 인터페이스의 MAC 주소를 조사해서 MAC 헤더에 기록하고 송신함
- 프로바이더끼리는 서로 계약 조건을 합의한 상대하고만 경로 정보를 교환하고 패킷을 주고 받음
- 정리하자면 프로바이더 상호간을 직접 접속하던 IX를 경유하던 최종적으로 패킷은 서버측의 프로바이더에게 도착하고 그곳의 POP을 경유하여 서버측의 네트워크로 흘러가는 것임
